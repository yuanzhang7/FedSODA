...
import random
from monai.transforms import (Orientationd, ScaleIntensityRanged, RandGaussianNoised,
                              RandCropByPosNegLabeld, RandAffined, Rand2DElasticd,
                              GaussianSmoothd)
...
def transform_img_lab(image, label, args):
    ...
    scale_shift = ScaleIntensityRanged(
        ...
        b_max=10,
        clip=True)
    gauss_noise = RandGaussianNoised(
        keys=["image"],
        prob=1.0,
        mean=0.0,
        std=1)
    gauss_smooth = GaussianSmoothd(
        keys=["image"],
        sigma=0.6,
        approx='erf')
